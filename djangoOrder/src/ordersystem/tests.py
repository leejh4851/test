...
def create_destination(a_customid, a_name, a_tel, a_postcode, a_address, a_detailAddress="",a_extraAddress=""):
    return Destination.objects.create(
        customid=a_customid,name=a_name,tel=a_tel,postcode=a_postcode,
        address=a_address,detailAddress=a_detailAddress,extraAddress=a_extraAddress)
   
class tests(TestCase):
    def test_destination(self):
        response = self.client.get(reverse('ordersystem:postal'))
        self.assertEqual(response.status_code, 200)
#         
#     def test_board(self):
#         response = self.client.get(reverse('boardEx:list2'))
#         self.assertEqual(response.status_code, 200)
#     
    def test_destination_exists(self):
        create_destination('sjw', '성주원', '010-1111-2222', '34667','대전 동구 판암동 750')
        response = self.client.get(reverse('ordersystem:postal'))
        self.assertEqual(response.status_code, 200)
        self.assertQuerysetEqual(
            response.context['destinationList'],['<Destination: 성주원>']
        )
#     
#     def test_board2_exists(self):
#         create_board(1234, '제목', '내용', '이름', timezone.now())
#         response = self.client.get(reverse('boardEx:list2'))
#         self.assertEqual(response.status_code, 200)
#         self.assertQuerysetEqual(
#             response.context['boardList'],
#             ['<Board: 제목>']
#         )